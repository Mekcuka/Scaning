...
from network_planner.plan.pipeline import (  # noqa: E402
...
def scenario_3_nodes_backbone():
    ...
    return "D. 3 nodes + 2 terminals on backbone", _metrics(_plan(req))
...
def scenario_terminal_on_start_node():
    ...
    return "E. terminal snapped to start node", _metrics(_plan(req))


def scenario_short_connector_max():
    req = build_request(3)
    req.options = PlanOptions(max_points=50, connector_max_km=0.001)
    return "F. connector_max_km=0.001 (1m limit)", _metrics(_plan(req))
...
def scenario_wide_zigzag():
    ...
    return "G. zigzag amplitude x3 (3 km)", _metrics(_plan(req))
...
def scenario_tight_spacing():
    ...
    return "H. segment spacing 2 km not 5 km", _metrics(_plan(req))
...
def main() -> None:
    ...
    print("Vhod -> plan (GeoSteiner or SteinerPy) -> polya otcheta (kak n03.json:9-13)\n")
