...
from network_planner.steiner.geosteiner import is_geosteiner_available  # noqa: E402
from network_planner.steiner.geosteiner.solver import solve_steiner_tree_geosteiner  # noqa: E402
from network_planner.steiner.steinerpy import is_steinerpy_available  # noqa: E402
from network_planner.steiner.steinerpy.solver import solve_steiner_tree_steinerpy  # noqa: E402

N = 9


def _steiner_tree_4(gids: list[str], local_pts: list[tuple[float, float]]):
    if is_geosteiner_available():
        return solve_steiner_tree_geosteiner(gids, local_pts, normalize_leaves=False)
    if is_steinerpy_available():
        return solve_steiner_tree_steinerpy(gids, local_pts, normalize_leaves=False)
    raise RuntimeError("Need GeoSteiner or SteinerPy for cluster scenarios")
...
def steiner_sweep_clusters(k: int) -> tuple[PlanRequest, PlanResponse]:
    ...
    for c in range(m):
        ...
        tree4 = _steiner_tree_4(gids, local_pts)
        rename: dict[str, str] = {}
...
def steiner_sweep_scenario(k: int) -> tuple[str, str, PlanRequest, PlanResponse, str]:
    ...
    if k == 1:
        ...
        return f"S{k:02d}", f"St={k} real zigzag n=3", req, resp, "solver"
    if k == 2:
        req, resp = steiner_sweep_real(2)  # type: ignore[misc]
        return f"S{k:02d}", f"St={k} real zigzag n=4", req, resp, "solver"
    if k % 2 == 0:
        ...
        return (
            ...
            req,
            resp,
            f"{k // 2} x SMT-4 (GeoSteiner/SteinerPy)",
        )
...
"""
    ...
    sweep_html: list[str] = [
        ...
        '<p class="note">St=1,2 — реальный расчёт (зигзаг). '
        "Чётные St≥4 — склейка кластеров SMT на 4 терминала (GeoSteiner/SteinerPy). "
