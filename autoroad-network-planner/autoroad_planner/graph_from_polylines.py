...
from autoroad_planner.schemas import ExistingAutoroadInput
from autoroad_planner.road_graph import RoadGraph, add_undirected_edge, haversine_km


class _PolylineEdgeRef:
    """Minimal edge handle so ``shortest_path_edges`` can report ``road.id``."""

    def __init__(self, road_id: UUID) -> None:
        self.id = road_id
...
def build_graph_from_polylines(roads: list[ExistingAutoroadInput]) -> RoadGraph:
    ...
    for road in roads:
        ...
        for i in range(len(node_ids) - 1):
            ...
            if w > 0:
                add_undirected_edge(g, a, b, w, db_edge=_PolylineEdgeRef(road.id))
    return g
