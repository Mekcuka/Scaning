...
| `POST /v1/design/horizontal` | 200 / 400 | ГС: build до точки входа + hold вдоль heel→toe; `entry_mode`: `any` / `heel` / `toe` |
...
Сегменты: **build** до точки входа на линии heel→toe, затем **hold** вдоль оси ГС (при входе между концами — entry→toe→heel). Режим `any` перебирает смещения и выбирает минимальную `geometry.length_m`. Ответ дополнительно: `entry_mode`, `entry_offset_m`, `entry_plan`.
