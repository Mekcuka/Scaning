...
Подробное сравнение используемого и доступного API обеих библиотек: [well-trajectory-libraries-coverage.md](../../docs/features/well-trajectory/well-trajectory-libraries-coverage.md).
...
| `POST /v1/design/horizontal` | 200 / 400 | ГС: build до точки входа + hold вдоль Т1→Т3; `entry_mode`: `any` / `Т1` / `Т3` |
...
### Пример: горизонтальный участок ГС

`POST /v1/design/horizontal`

```json
{
  "start": { "northing": 0, "easting": 0, "tvd": 0, "inc": 0, "azi": 90 },
  "heel": { "northing": 500, "easting": 200, "tvd": 2400, "inc": 90, "azi": 90 },
  "toe": { "northing": 500, "easting": 1200, "tvd": 2500, "inc": 90, "azi": 90 },
  "step_m": 30,
  "inc_heel": 90,
  "entry_mode": "any",
  "entry_search_step_m": 30,
  "azi_reference": "grid"
}
```

Сегменты: **build** до точки входа на линии Т1→Т3 (только **Т1** или **Т3**), затем **hold** вдоль оси ГС до противоположного конца. Режим `any` сравнивает только два конца и выбирает минимальную `geometry.length_m`. Ответ дополнительно: `entry_mode`, `entry_offset_m`, `entry_plan`.

Фильтрация по SF при `any` выполняется в BFF монолита (`design-from-bottomholes`), не в микросервисе.
...
Обёртки BFF описаны в [well-trajectory.md](../../docs/features/well-trajectory/well-trajectory.md).
...
| Тема | Файл |
|------|------|
| Функция в продукте | [well-trajectory.md](../../docs/features/well-trajectory/well-trajectory.md) |
| Оценка текущего приложения | [well-trajectory-app-assessment.md](../../docs/features/well-trajectory/well-trajectory-app-assessment.md) |
| Модель данных | [well-trajectory-data-model.md](../../docs/features/well-trajectory/well-trajectory-data-model.md) |
| План работ | [well-trajectory-roadmap.md](../../docs/features/well-trajectory/well-trajectory-roadmap.md) |
| 2D-раскладка устьев | [pad-earthwork.md](../../docs/features/pad-earthwork/pad-earthwork.md) |
