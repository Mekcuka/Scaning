...
- **Горячие клавиши** (на странице карты, не в полях ввода): **E** — вкл/выкл «Редактирование на карте»; **Del** / **Backspace** — удалить выбранное; **Ctrl+Z** — отмена последнего действия; **Enter** — завершить черновик линии; **Escape** — закрыть модал/поиск или выйти из режима рисования; **двойной ЛКМ** по промежуточной вершине линии — удалить вершину.
- **Редактирование линий:** включите «Редактирование на карте» → «Выбор» → выберите линию. Перетаскивание вершин; **двойной ЛКМ** по промежуточной вершине — удалить (концы линии не удаляются). Подсказки — в footer карты.
...
- **2.5D / 3D карта:** в `frontend/.env`: `VITE_MAP_3D_ENABLED=true` и `VITE_MAPTILER_KEY=<ключ MapTiler>`. Перезапустите `npm run dev`. На `/map` — **2D | 3D**; в слоях — спутник, **Рельеф (3D)**, **3D-модели** (glTF), фильтры подтипов. Рисование только в 2D. Точки: glTF Kenney + палитра цвета слоя; линии: 3D-трубы по рельефу. Высоты: вкладка «Дополнительно» или `render_3d_*`; импорт: `height_m`, Z → `render_3d_base_m`. Backfill: `python scripts/backfill_render_3d_properties.py --dry-run`. Документация: [docs/map-3d-features.md](../docs/map-3d-features.md).
