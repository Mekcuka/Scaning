...
export async function applyInfraLineSplit(params: {
  ...
  networkApi?: NetworkBuildApiPort;
}): Promise<{ updated: InfraObject; second: InfraObject } | null> {
  const { projectId, split, splitLon, splitLat } = params;
  const mapApi = params.mapApi ?? defaultMapMutationsApi;
  const networkApi = params.networkApi ?? defaultNetworkBuildApi;
  ...
  const updated = await mapApi.updateInfraObject(projectId, split.line.id, plan.firstPayload);
  const second = await mapApi.createInfraObject(projectId, {
    ...plan.secondPayload,
  ...
  try {
    await networkApi.buildNetwork(projectId);
  } catch {
    /* best-effort */
  }
