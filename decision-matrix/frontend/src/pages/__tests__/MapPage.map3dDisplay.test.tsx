...
describe('MapPage 3D display optimization', () => {
  ...
  it('unmounts 3D map when switching back to 2D', async () => {
    await renderMap();

    await userEvent.click(screen.getByLabelText('Карта 3D'));
    await waitFor(() => expect(screen.getByTestId('mock-map-3d')).toBeInTheDocument());
    const mountsAfter3d = map3dPropsRef.mountCount;

    await userEvent.click(screen.getByLabelText('Карта 2D'));
    await waitFor(() => expect(screen.queryByTestId('mock-map-3d')).not.toBeInTheDocument());
    expect(map3dHost()).toBeNull();

    await userEvent.click(screen.getByLabelText('Карта 3D'));
    await waitFor(() => expect(screen.getByTestId('mock-map-3d')).toBeInTheDocument());
    expect(map3dPropsRef.mountCount).toBeGreaterThan(mountsAfter3d);
  });
});
