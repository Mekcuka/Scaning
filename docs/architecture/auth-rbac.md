...
| Production | `DEMO_USERS_ENABLED=false`, уникальный `SECRET_KEY`; `ALLOW_REGISTRATION` — по выбору оператора (в deploy-шаблонах `true`) |
...
| Custom GLB 404 на карте после upload | Ctrl+F5; на VM: `ls /opt/decision-matrix/shared/map3d_models/<project_id>/`; mount и миграция `022` — [map3d-models-storage.md](../deploy/map3d-models-storage.md), [map-3d-features.md](../features/map/map-3d-features.md) §12 |
