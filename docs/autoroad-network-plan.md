...
| **Steiner-узел** | `junction` на геодезической середине ребра MST; перекрёстки только в `●`, не в `Т` |
...
4. **Дорог нет, 2 объекта** — одна автодорога между ними (узел между ними не обязателен).
5. **Дорог нет, 3+ объекта** — **MST:** `n−1` узлов `●` на рёбрах MST, `link` между узлами у общих терминалов, по одному `connector` от каждого `Т`; несколько дорог в одной точке `Т` **запрещено**.
6. **Уже подключён** — конец существующей дороги в **≤20 m** от объекта → второй подъезд не строят (`already_connected`).
7. **Далеко от дороги** — snap **>0,3 km** → предупреждение `far_from_autoroad`, подъезд всё равно строят.
8. За раз **от 2 до 50** терминалов; POI и подтипы `node` / `methanol_joint` / `power_line_node` не участвуют.
...
Степень 2 в MST: `link` между соседними `●`, `connector` на магистраль. Степень ≥3: `Т→J_T`, лучи `J_T→●` (без хорды `●₁—●₂` через угол).
...
| А.1 | 3+ `Т`, нет дорог | **MST** по расстояниям между `Т`; узел `●` на **каждом** ребре MST |
...
2. Три и больше → MST по `Т` → `●` на ребре → степень 2: `link` `●—●` + `connector` на магистраль; степень ≥3: hub `J_T`, `connector` `Т→J_T`, звезда `J_T→●` → `_simplify_collinear_backbone` → **`_repair_planned_line_topology`** (примыкание под 90° к середине сегмента: отдельный `●` и два `link`) → `acute_bend_deg` в preview.
...
| Метод | Путь | Действие |
|-------|------|----------|
| POST | `/api/v1/projects/{id}/autoroad-network/plan` | Снимок проекта → JSON плана |
| POST | `/api/v1/projects/{id}/autoroad-network/apply` | План → сохранение (+ job при очереди) |

Устаревший: `POST .../infrastructure/autoroad-connect`.

### API планировщика (внутренний / отдельный сервис)

**POST** `/v1/network/plan`

**Вход:** `terminals`, `existing_autoroads`, `options`.
