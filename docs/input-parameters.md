...
**Связанные документы:** [requirements.md](./requirements.md), [database-schema.md](./database-schema.md), [map-objects-and-spatial-calculations.md](./map-objects-and-spatial-calculations.md), [calculation-functions.md](./calculation-functions.md), [prototype-alignment.md](./prototype-alignment.md), [consistency-review.md](./consistency-review.md).
...
| **UI** | Экран / элемент прототипа `Cursor_Scan/` |
...
Одна строка на пару `(project_id, subtype)` в `project_cost_rates`. В прототипе ключи совпадают с `id` строки в `js/cost-rates-spec.js`.

#### Линейные внутренние (`rate_unit = per_km`, тыс. ₽/км)

| id | Статус | Подтип БД | По умолчанию (прототип) | UI | FR |
|----|--------|-----------|-------------------------|-----|-----|
...
| id | Статус | Тип | Единица | БД | UI (прототип) | FR | Влияет на расчёт |
|----|--------|-----|---------|-----|---------------|-----|------------------|
...
| id | Статус | Тип | Значения БД | UI (прототип) | FR | Влияет на расчёт |
|----|--------|-----|-------------|---------------|-----|------------------|
...
Значения `analysis_distance_source` (MVP): `geodesic` (внешние), `pads_per_pad_formula` (внутренние линейные), `manual_override` (сценарий).
...
## §5. Варианты и сценарии

| id | Статус | Тип | БД | UI | FR |
|----|--------|-----|-----|-----|-----|
| `variant_name` | mvp | string | `implementation_variants.name` | Матрица, модал сценария | FR-7.2 |
| `variant_type` | mvp | enum | `variant_type` | `base` / `scenario` | FR-7.1, FR-7.2 |
| `variant_parent_id` | mvp | uuid | `parent_variant_id` | — | FR-7.2 |
...
| `variant_distance_km_override` | mvp | number | `variant_infrastructure_items.distance_km` (override) | Сценарий, internal/external | FR-6.3.5 |
| `variant_km_per_pad_override` | mvp | number | snapshot в `applied_params` | Сценарий, internal | FR-6.3.5 |

**Вычисляемые поля варианта:** `variant_total_cost`, `variant_engineering_equipment_cost`, `variant_overall_status` (FR-7.3.5–7.3.6, FR-6.2.3).

---

## §6. Ранжирование сценариев (FR-9)

### §6.1 Критерии

| id | Статус | Тип критерия | `value_source` | Ввод пользователя | FR |
|----|--------|--------------|----------------|---------------------|-----|
| `rank_criterion_cost` | mvp | cost | `computed` | нет | FR-9.2.1 |
| `rank_criterion_distance` | mvp | cost | `computed` | нет | FR-9.2.1 |
| `rank_criterion_exceed_count` | mvp | cost | `computed` | нет | FR-9.2.1 |
| `rank_criterion_risk` | mvp | cost | `user` | 1–10 по каждому сценарию | FR-9.2.4 |
| `rank_criterion_time` | mvp | cost | `user` | месяцы ≥ 1 | FR-9.2.4 |
| `rank_criterion_reliability` | mvp | benefit | `user` | 1–10 по каждому сценарию | FR-9.2.4 |

Хранение экспертных оценок: `scenario_criterion_values` (`variant_id`, `criterion_id`, `value`).

### §6.2 Настройки матрицы ранжирования

| id | Статус | Тип | БД / UI | FR |
|----|--------|-----|---------|-----|
| `rank_criterion_weights` | mvp | json | `project_ranking_settings.weights`, сумма = 1.0 | FR-9.0, FR-9.2.3 |
| `rank_algorithm` | mvp | enum | `topsis`, `wsm`, `ahp` | FR-9.3.1–9.3.2 |
| `rank_ahp_pairwise` | mvp | json | Парные сравнения AHP | FR-9.3.1 |
| `rank_default_expert_values` | mvp | json | `default_expert_values` | FR-9.2.6 |

Пример `default_expert_values` (FR-9.2.6):
...
| id | Статус | Тип | БД | UI (прототип) | FR |
|----|--------|-----|-----|---------------|-----|
...
| `one_pager_variant_id` | mvp | uuid | `variant_id` | Выбор сценария | FR-11.1.1 |
...
- **id (предлагаемый):** 
- **Уровень:** проект | POI | вариант | импорт | ранжирование
- **Тип / единица:** 
- **Зачем (бизнес):** 
...
4. [database-schema.md](./database-schema.md) и [prototype-alignment.md](./prototype-alignment.md).
5. Запись в [consistency-review.md](./consistency-review.md).

---

## §11. Соответствие прототипу `Cursor_Scan/`

| Каталог `id` | Прототип / модуль | Примечание |
|--------------|-------------------|------------|
| `rate_*` | `cost-rates-spec.js` → `rates[id]` | localStorage `dm-cost-rates-{projectId}` |
| `poi_*`, `eng_*` (форма) | `index.html`, `app.js` `readPoiFromForm()` | Бейджи в проекте — статичный HTML |
| `eng_*` (матрица) | `matrix-spec.js` `engKey` | `applyEngineeringRules()` |
| `threshold_*` (внешние 4) | — | **mvp** |
| `max_total_line_*` | — | **mvp** |
| `km_per_pad_*` | — | **mvp** |
| `rank_default_expert_values` | — | **mvp** |
| `rank_criterion_*` (user) | — | **mvp** |
| `import_*` | `index.html` `#view-import` | Mock, без API |
...
| 2026-05 | `km_per_pad_*` (4); `analysis_distance_source`; internal linear = pads × km/КП; threshold_* → mvp; ranking expert → mvp; `project_visibility` |
| 2026-05 | `max_total_line_*` (4); пороги external/internal разделены; FR-9.0.4 base в ранжировании; FR-9.2.6 дефолты 5/5/12 |
