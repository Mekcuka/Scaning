...
- ГИС-картой (**OpenLayers**; HTML-прототип в [`../Cursor_Scan/`](../Cursor_Scan/))
- Управлением проектами инфраструктуры
- Матрицами решений (TOPSIS, WSM, AHP)
- Анализом окружения точек интереса
- Расчётом стоимости вариантов реализации

> **Актуализация прототипа (2026):** см. [prototype-alignment.md](./prototype-alignment.md) — отдельные разделы «Импорт» и «Ставки», вертикальная матрица + карточный вид, OpenLayers.
...
| [prototype-alignment.md](./prototype-alignment.md) | **Соответствие HTML-прототипа `Cursor_Scan/`** |
...
- **OpenLayers** (прототип и целевой GIS-клиент; см. `Cursor_Scan/js/map-ol.js`)
- ~~Leaflet~~ — не используется в текущем прототипе
- **Recharts** - графики
- **Lucide React** - иконки
...
**Демо-вход:** `engineer@oilgas.ru` / `password123` (после `python seed.py` или `run_local.py`)
...
## Алгоритмы матриц решений

### TOPSIS
Technique for Order Preference by Similarity to Ideal Solution - метод приближения к идеальному решению.

### WSM
Weighted Sum Model - взвешенная сумма.

### AHP
Analytic Hierarchy Process - аналитический иерархический процесс.
