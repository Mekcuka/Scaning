...
> **См. также:** [well-trajectory.md](../features/well-trajectory.md), [well-trajectory-data-model.md](well-trajectory-data-model.md), [well-trajectory-roadmap.md](well-trajectory-roadmap.md), [input-parameters.md](../product/input-parameters.md), [pad-earthwork.md](../features/pad-earthwork.md), [pad-placement-optimization.md](../features/pad-placement-optimization.md) (✅).
...
| Настройки welleng на кусте (вкладка «Расчёт») | **Есть** — 7 параметров, см. §4.5 |
...
**Итог:** для MVP траекторий опираемся на раскладку устьев, KB, DEM и **настройки расчёта на кусте** (§4.5). Пробелы: WITSML import (4b), `default_target_tvd_m` в design, `step_m` в карточках, настройки проекта на уровне проекта, E2E pad-placement. **Multi-pad placement optimizer** — ✅ [pad-placement-optimization.md](../features/pad-placement-optimization.md) (M1–M5 + M2+ center optimize), отдельно от per-pad «Кустование» и от POI `pads_count`.
...
| Параметры connector (DLS, тип J/S) | welleng | Нет — фиксированный connector между точками |
| Multi-lateral / разветвления | PyWellGeo | Нет |
...
| Функция | Статус |
|---------|--------|
| `.../well-trajectory/*` | **Есть** — см. [well-trajectory.md](../features/well-trajectory.md) |
