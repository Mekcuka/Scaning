...
**Привязка к сети:** перед расчётом на карте вызывается **«Построить сеть»** (`build_network_from_lines`); точечный объект участвует в подсети только если он **не дальше 0,3 км от геометрии линии** `autoroad` (как при привязке концов линии на карте), затем привязывается к ближайшему узлу этой автодороги. Объекты вне дорог (например, ДНС без подъезда) **не попадают** в схему, таблицы подсети и расчёт — только в предупреждения (`too_far_from_autoroad`). Кратчайшие пути — **Dijkstra** по рёбрам `autoroad`.
