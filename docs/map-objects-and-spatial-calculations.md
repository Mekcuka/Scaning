...
Код: `app/geo/sand_properties.py`, `app/services/sand_logistics.py`, frontend `lib/infraSandVolumes.ts`, вкладка **Потоки → Логистика**. После расчёта таблица **«Плечо возки»** (карьер, объём, км) — во вкладке **Логистика** карточки потребителя на карте (`ObjectDetailPanel`, `lib/sandLogisticsHaulLegs.ts`).
