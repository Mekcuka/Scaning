...
| Точечные потребители (все подтипы **кроме** `node`, `sand_quarry`) | `sand_volume_m3` | м³ (спрос) |
...
Код: `app/geo/sand_properties.py`, `app/services/sand_logistics.py`, frontend `lib/infraSandVolumes.ts`, вкладка **Потоки → Логистика**.
...
В **логистике песка** в расчёт попадают только объекты и участки автодорог с `entry_date ≤ дата расчёта` (сегодня). Остальные отображаются в таблицах с пометкой «не введён», без отгрузки.
...
| **Двойной ЛКМ** | Удалить промежуточную вершину выбранной линии (не концы) |
