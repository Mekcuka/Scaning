...
- **Рисование линии** (инструмент «Линия», только 2D):
  ...
  - **Завершение** — двойной **ЛКМ** / двойной **ПКМ**, **Enter** или кнопка «Готово»; позиция конца из последнего клика/курсора. Если конец ≤300 м от объекта — привязка к его `lon`/`lat`; иначе создаётся **`node`** в этой точке, затем сохраняется линия.
