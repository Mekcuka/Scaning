...
3. **Узел доступа** на **50 м** (`access_node_offset_km`) от объекта по направлению к snap или к центроиду остальных терминалов — будущий `subtype=node` (`reason=terminal_access`). Код: [`access_nodes.py`](../decision-matrix/backend/app/services/autoroad_network/access_nodes.py).
4. **MST** по координатам узлов доступа: если оба терминала на одной сети — путь по **существующим** `autoroad` (`used_existing_edge_ids`); иначе новый `link` между узлами (`snap_*_terminal_id`).
5. **Подъезды:** `connector` объект→узел (всегда); при snap и расстоянии узел—snap **>20 м** — `network_tie` узел→точка на дороге.
...
| Дата | Изменение |
|------|-----------|
| 2026-06 | Автосеть: узел доступа 50 м перед каждым терминалом (объект→узел→дорога→узел→объект) |
