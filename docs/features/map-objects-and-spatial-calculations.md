...
**Режим карты «Оптимизация кустов»** ([pad-placement-optimization.md](pad-placement-optimization.md)):** на входе — выбранные забои (ННБ, пятка и сток ГС); после применения — **новые** кустовые площадки, существующие не меняются. Перебор центра по Σ MD — блок «Расширенные» (двухфазный: грубая сетка до 5×5, затем полный design). Не путать с **«Кустованием»** (один куст) и с подтипом `pads` в матрице POI (только число для стоимости, FR-5.3.1).
