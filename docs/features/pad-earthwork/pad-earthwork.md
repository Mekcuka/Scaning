...
**Генератор скважин** (автоконтур по ряду скважин) — только `oil_pad` / `gas_pad`. Остальные объекты: схема «Произвольная» / «Прямоугольник» в модалке «Схема…». Тот же генератор доступен на странице навигации **«Кустование»** (`/pad-clustering`).

**Связанные функции:** [Траектории скважин (3D)](../well-trajectory/well-trajectory.md) — **M2 ✅, M3 ✅**: «Кустование», design до забоев, **anti-collision SF**, GeoJSON на карте; координаты устьев — `pad_wells_local_json`. [Оптимизация размещения кустов](../pad-placement/pad-placement-optimization.md) (✅) — переиспользует **генератор раскладки** (`generate-from-wells`) для каждого кандидата куста; M2+ — **двухфазный** перебор центра по Σ MD. [Оценка](../well-trajectory/well-trajectory-app-assessment.md). [Roadmap](../well-trajectory/well-trajectory-roadmap.md).
