...
**Статус:** реализовано (май 2026). Режим **только просмотр**; редактирование геометрии — в **2D** и **Площадки** (OpenLayers).
...
Переключатель **2D | Площадки | 3D** в toolbar карты. При открытии вкладки «Карта» по умолчанию активен режим **2D**; режим **3D** включается вручную. **Площадки** — контуры footprint на OpenLayers (редактирование доступно); см. [pad-earthwork.md](../pad-earthwork/pad-earthwork.md) § «Режим „Площадки“ на карте».
...
|------------|------------|
| `VITE_MAP_3D_ENABLED` | `true` — показать переключатель 2D / Площадки / 3D |
...
- Объекты инфраструктуры `well_bottomhole_*` (ННБ, ГС Т1/Т3, unified ГС) — те же сферы/трубы в Three.js по KB−TVD; при включённом слое забоев не дублируются на земле в MapLibre.
...
См. также [well-trajectory.md](../well-trajectory/well-trajectory.md) § «3D-карта».
...
| Редактирование | только **3D** недоступно; **2D** и **Площадки** — OpenLayers |
| Отчёт PNG | только 2D |
