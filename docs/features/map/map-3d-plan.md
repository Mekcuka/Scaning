...
**Дата:** май 2026  
**Связанные документы:** [map-3d-features.md](map-3d-features.md) (**реализованное состояние**), [map-objects-and-spatial-calculations.md](map-objects-and-spatial-calculations.md), [architecture.md](../../architecture/architecture.md), [database-schema.md](../../architecture/database-schema.md)
...
|-------|------------------|--------------------|--------------|
| **Ручное рисование** | MapPage → «Точка» / «Линия» / «POI» (только 2D); линия — §2.9, map-objects §1.5 | `manual` | map-objects §1.4 |
...
| `power_line_node` | 10 | как `power_line` (опора ЛЭП) |
| `sand_quarry` | 5 | |
...
### 2.9 Линии: привязка концов, рисование, согласование 2D/3D (выполнено, май 2026)

> Детали: [map-objects-and-spatial-calculations.md](map-objects-and-spatial-calculations.md) §1.5, §6.1; [map-3d-features.md](map-3d-features.md) §6.

**Привязка и хранение**

- [x] **D-L1** `normalizeLinePathEndpoints` / `linePathForDisplay` — концы линии = точные `lon`/`lat` точечного объекта (совпадение координат)
- [x] **D-L2** Backend `snap_line_endpoints_to_point_objects` при create/update + тесты
- [x] **D-L3** `infraSnapPool` на карте (все объекты проекта, не только фильтр слоёв)
- [x] **D-L4** Heal устаревших концов при загрузке (`lineEndpointHealPayload`, опциональный PATCH)
- [x] **D-L5** Координаты: полная точность в БД/API; в UI — 3 знака (`coords.ts`, `coordForSave`)

**Рисование (только 2D)**

- [x] **D-L6** Начало линии — на точечном объекте; промежуточные вершины — свободно
- [x] **D-L7** Завершение: двойной ЛКМ/ПКМ, Enter, «Готово»; конец на объекте или авто-`node`
- [x] **D-L8** Редактирование: `constrainLineCoordinatesOnEdit`, нельзя оставить конец «в воздухе»

**3D-отрисовка линий**

- [x] **D-L9** Трубы (`map3dLineMeshes.ts`): прямые сегменты `LineCurve3` / `CurvePath` — план = 2D `LineString` (без Catmull-Rom)
- [x] **D-L10** `buildNormalizedLinePath3d` + общий snap-пул для tubes и GeoJSON pick
- [x] **D-L11** ЛЭП: провода по прямым пролётам в плане; высота коридора отдельно от визуала опор
- [x] **D-L12** glTF точек: центрирование по XZ (`map3dModelsLayer.ts`)

**Тесты**

- [x] **D-L13** Vitest: `lineEndpointRules`, `infraGeometry`, `mapLineDrawingRules`, `map3dLineMeshes`, `map3dLinePathBuild`
...
- [x] **T4.1** map-objects-and-spatial-calculations.md — §1.5 (линии, координаты), §6.1 (hotkeys), §6.4 (3D)
- [x] **T4.2** user-flows.md — канал В (рисование линии)
- [x] **T4.1b** map-3d-features.md §6 (линии 2D/3D), spark-import, requirements FR-2.1.4, RUN_GUIDE §7
...
│   └── MapView3D.tsx
├── lib/
│   ├── coords.ts                    # COORD_DECIMALS, coordForSave
│   ├── infraGeometry.ts             # linePathForDisplay, heal
│   ├── lineEndpointRules.ts         # snap, draw, edit constraints
│   ├── map3d/
│   │   ├── geoJson.ts
│   │   ├── geoJson.test.ts
│   │   ├── extrusionHeights.ts
...
│   │   ├── map3dLayers.ts
│   │   ├── map3dBasemap.ts
│   │   ├── map3dLineMeshes.ts       # tubes: CurvePath + LineCurve3
│   │   ├── map3dLinePathBuild.ts    # normalized path + alts
│   │   ├── map3dLinesLayer.ts
│   │   ├── map3dPowerLineMeshes.ts
│   │   └── map3dModelsLayer.ts
│   └── mapViewState.ts
...
- [x] Линия с ≥3 вершинами в 3D совпадает с 2D (прямые сегменты, без «зеркального» изгиба)
- [x] Концы линии в 3D совпадают с центром привязанной точечной модели (snap + glTF center)
