...
**Режим карты «Оптимизация кустов»** ([pad-placement-optimization.md](../pad-placement/pad-placement-optimization.md)):** на входе — выбранные забои (ННБ, Т1 и Т3 ГС); после применения — **новые** кустовые площадки, существующие не меняются. Перебор центра по Σ MD — блок «Расширенные» (двухфазный: грубая сетка до 5×5, затем полный design). Не путать с **«Кустованием»** (один куст) и с подтипом `pads` в матрице POI (только число для стоимости, FR-5.3.1).
...
| `methanol_facility` | Объект метанола | **Точка** | `POINT` | `area_facility` | **Точка** / импорт Искра | — | нет | — | — |
...
| `well_bottomhole_gs` | ГС (unified) | **Линия** | `LINESTRING` | `well` | «Забой» → ГС (Т1→Т3) | — | нет | — | — |
| `well_bottomhole_gs_heel` | ГС — Т1 | **Точка** | `POINT` | `well` | «Забой» → ГС (legacy, 1-й клик) | — | нет | — | — |
| `well_bottomhole_gs_toe` | ГС — Т3 | **Точка** | `POINT` | `well` | «Забой» → ГС (legacy, 2-й клик) | — | нет | — | — |
...
- **Режим «Площадки» — ребро footprint (display-only):** на earthwork-**точке** `properties.footprint_line_connections` (подтип линии → `edge_index`, `t` или `null` для центра) смещает **только отображение** конца линии к ребру контура; шаблон проекта (cardinal) — **Параметры → Точки подключения**, таблица `project_footprint_connection_templates`. Legacy `properties.line_footprint_attach` на линиях — fallback. Модуль [`padFootprintLineAttach.ts`](../../decision-matrix/frontend/src/lib/padFootprintLineAttach.ts); см. [pad-earthwork.md](../pad-earthwork/pad-earthwork.md), wiki [parameters-footprint-connections.md](../../wiki/articles/parameters-footprint-connections.md).
...
- **Импорт Искра:** полигоны площадных типов → `POINT` (центроид); см. [spark-import-mapping.md](../import-export/spark-import-mapping.md).
...
Подтипы траекторий скважин на карте; manifest — `decision-matrix/shared/infrastructure_subtypes.json`, кластер `clusters.bottomhole`. Подробнее — [well-trajectory.md](../well-trajectory/well-trajectory.md), [input-parameters.md](../../product/input-parameters.md) §3.1.
...
| `well_bottomhole_gs` | `LINESTRING` (≥2 вершины) | Линия Т1→Т3 | Unified ГС: dual TVD (`heel_tvd_m`, `toe_tvd_m`), `gs_entry_mode`; **не** в `point.map` |
...
**Роли забоя (property, не отдельный subtype):** цепочка **Куст → основной забой → доп.ствол**.

| Ключ | Значения | Назначение |
|------|----------|------------|
| `well_bottomhole_role` | `main` (default) \| `lateral` | Основной забой / доп.ствол |
| `well_bottomhole_parent_id` | UUID | Для `lateral`: ссылка на основной забой; куст и `well_index` наследуются |

- **Основной забой** (`main`): `well_bottomhole_linked_pad_id` → куст; участвует в `sync-bottomholes` как primary target основного ствола; в подсчёте слотов скважин (`slot_demand`) учитывается как отдельная скважина.
- **Доп.ствол** (`lateral`): привязка только через `parent_id` к **основному** забою (`role=main`); `linked_pad_id` и `well_index` **наследуются** от родителя (не задаются вручную). В `sync-bottomholes` **не** материализует primary target — только цель боковых веток PyWellGeo («До забоя»).
- **Карта (2D):** доп.ствол — маркер **фиолетовый** (`#7b1fa2`); пунктир **parent → lateral** (`lateral-bottomhole-connector`), стиль как у связи ГС Т1–Т3. **Пунктир скрывается**, если для пары уже есть рассчитанная ветка PyWellGeo (`pywellgeo_branch` в GeoJSON траекторий): конец ветки совпадает с целью доп.ствола (NNB или Т1/Т3 ГС) в пределах **75 м** на том же кусте и том же `well_index`.
- **Кустование (3D):** тот же пунктир parent→lateral; скрывается по preview-веткам PyWellGeo (`pywellgeoSegments`) для выбранной скважины.
- **Рисование:** меню «Забой» → **Доп.ствол (ННБ/ГС)** — автопривязка к ближайшему основному забою на кусте; роль и родителя можно сменить в карточке объекта.
- **Карточка забоя:** поле «Роль» (Основной / Доп.ствол); для lateral — выбор «Родительский забой», куст и «Скв. №» read-only.
- **Кустование → таблица забоев:** колонка «Роль», иерархический список (доп.стволы с отступом под основным).

**Валидация (backend):** lateral без `parent_id` — ошибка; main с `parent_id` — ошибка; lateral→lateral запрещён; удаление main при наличии дочерних lateral — блокировка. Код: `bottomhole_validation.py`.

**Валидация геометрии:** `validate_subtype_geometry` — point-подтипы требуют одну координату; `well_bottomhole_gs` — линию (`coordinate_count ≥ 2`). Тест: `tests/test_subtype_manifest.py::test_well_bottomhole_subtypes_geometry_manifest`.
...
**Роль на схеме потоков ([fluid-flow-schematic.md](../flows/fluid-flow-schematic.md)):**
...
Код: `app/geo/sand_properties.py`, `app/services/sand_logistics.py`, frontend `lib/infraSandVolumes.ts`, вкладка **Логистика → Логистика**. После расчёта таблица **«Плечо возки»** (карьер, объём, км) — для **всех точечных объектов с полем спроса** (`sand_volume_m3`, кроме `node` и `sand_quarry`): вкладка **Логистика** карточки на карте, столбец на **Логистика → Объём песка**, колонка в таблице потребителей на **Логистика → Логистика** (`lib/sandLogisticsHaulLegs.ts`).
...
**Применение** (`dry_run: false`) выполняется **фоновой задачей** `autoroad_connect` (см. §1.9), не синхронно в HTTP.

#### 1.9 Фоновые задачи проекта (Redis + ARQ)

Тяжёлые операции ставятся в очередь **ARQ** (Redis); состояние хранится в PostgreSQL (`project_jobs`):

| `job_type` | Операция |
|------------|----------|
| `autoroad_connect` | Применение соединения автодорог |
...
На вкладке **Логистика → Логистика** для каждой подсети отображается интерактивная схема (`SandLogisticsFlowSchematic`, `lib/sandLogisticsFlow.ts`, `lib/sandLogisticsNodeVisual.ts`, `SandLogisticsSchematicTimeline`).
...
| 2026-06 | §1.4 / §1.4.5: объекты-забои `well_bottomhole_*` — unified ГС (`well_bottomhole_gs`) как `LINESTRING`, NNB/legacy Т1/Т3 как `POINT`; manifest `linear.all` vs `point.map` |
