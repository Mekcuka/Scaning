...
Переключатель **2D | Площадки | 3D** в toolbar карты. При открытии вкладки «Карта» по умолчанию активен режим **2D**; режим **3D** включается вручную. **Площадки** — контуры footprint на OpenLayers (редактирование доступно); см. [pad-earthwork.md](pad-earthwork.md) § «Режим „Площадки“ на карте».
