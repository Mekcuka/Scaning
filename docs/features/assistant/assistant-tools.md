...
| `GET` | `/api/v1/admin/assistant/llm-config` | Текущая конфигурация LLM (effective + env, embedding, wiki RAG, кэш probe 30 с) |
...
| `POST` | `/api/v1/admin/assistant/llm-probe` | Детальная проверка chat (`/models`, `/chat/completions`) и embeddings (`/embeddings`) |
| `POST` | `/api/v1/admin/assistant/llm-test` | Короткий тестовый completion (latency, echo модели) |
| `GET` | `/api/v1/admin/assistant/llm-models` | Список моделей с провайдера (`GET /models`, HTTP 200) |

Tool-аналог журнала: `admin_list_assistant_audit`.

#### UI: параметры LLM (admin)

Маршрут: **`/admin/assistant`** (вкладка **Администрирование → AI-помощник**, только роль `admin`).

**Структура страницы**

| Элемент | Назначение |
|---------|------------|
| Шапка + steps | «1. Статус → 2. Настроить → 3. Проверить» |
| Вкладка **Статус и настройка** | Метрики, probe, конфигурация, override |
| Вкладка **Справка и пресеты** | Провайдеры (Ollama, LM Studio, OpenRouter…), локальные пресеты, блок Cursor MCP |
| Toolbar | **Проверить подключение** (`POST /llm-probe`), **Обновить** (`GET /llm-config`) |

**Вкладка «Статус и настройка»**

| Блок UI | Назначение |
|---------|------------|
| Карточки статуса | LLM (`provider_ready`), веб-чат, Wiki RAG (embeddings / «только ключевые слова» / выключен), runtime override; цветовая полоска `ok` / `warn` / `bad` |
| **Детали проверки** | Сворачиваемая таблица probe: Chat `/models`, Chat `/completion`, Embeddings — HTTP-код, `hint_ru`, «Как исправить» для embeddings |
| **Текущая конфигурация** | Effective chat + embedding URL/model, max tokens, timeout, маскированный ключ; строки, отличающиеся от `.env`, подсвечены |
| **Временный override** | Подвкладки **Chat LLM** / **Wiki RAG / Embeddings**; combobox моделей из `GET /llm-models` (+ «Другая модель…»); max tokens, timeout, API key; embeddings — «как chat» или отдельный endpoint; **Тестовый запрос** → `POST /llm-test`; **Сохранить пресет** → `localStorage` (без секретов) |

При первой загрузке страница автоматически вызывает probe, если в `llm-config` ещё нет кэша `probe_detail` (TTL 30 с).

**Wiki RAG в UI:** при недоступных embeddings badge показывает «Поиск wiki: только ключевые слова»; кнопка **Как исправить** открывает модалку с шагами и **Подставить nomic-embed-text** в форму override.

**Вкладка «Справка и пресеты»**

| Блок UI | Назначение |
|---------|------------|
| Локальные пресеты | Чипы из `localStorage` — подстановка в форму override на вкладке «Статус» |
| Справка: провайдеры LLM | Аккордеон с карточками провайдеров; **Подставить** заполняет override |
| Cursor MCP | Сворачиваемый блок: `mcp_url`, TTL токена, `mcp_setup_hint_ru` из `GET /assistant/status` |

**Ссылка из чата:** если `provider_ready: false`, у admin в панели помощника показывается ссылка на `/admin/assistant`.

**Код frontend:** `frontend/src/pages/AdminAssistantPage.tsx`, компоненты `frontend/src/components/admin-assistant/*`, API — `frontend/src/lib/api/adminApi.ts`.

**Постоянные** настройки — переменные `ASSISTANT_LLM_*` и `ASSISTANT_WIKI_EMBEDDING_*` в `app.env` на VM и перезапуск контейнера API ([DEPLOY.md](../../DEPLOY.md)). **Временный** override действует только в памяти процесса до рестарта backend.

`GET /admin/assistant/llm-config` — поля: `provider_ready`, `chat_enabled`, `effective` (включая `max_tokens`, `timeout_seconds`), `embedding_effective`, `env`, `runtime_override`, `wiki_rag` (`rag_mode`, `embedding_model`, `embedding_ready`), `probe_detail` (после probe, TTL 30 с).

**Wiki RAG:** `rag_mode` = `disabled` | `tfidf` | `embedding` — при недоступных embeddings поиск wiki работает по ключевым словам (backend может использовать TF-IDF как fallback); исправление — отдельный `ASSISTANT_WIKI_EMBEDDING_BASE_URL` / `ASSISTANT_WIKI_EMBEDDING_MODEL` или embedding-модель на том же LM Studio/Ollama. Override embeddings — через admin UI или env.

Смены override и тест LLM пишутся в audit (`tool_name`: `admin_llm_config_update`, `admin_llm_config_reset`, `admin_llm_config_test`).

Если backend устарел (нет `GET /llm-config` или `POST /llm-probe`), UI показывает предупреждение; probe может вернуть **упрощённый режим** (статус из `llm-config`). Перезапустите `run_local.py` — см. [RUN_GUIDE.md](../../decision-matrix/RUN_GUIDE.md).
...
Скрипт логинится (по умолчанию `admin@oilgas.ru`), записывает `.cursor/mcp.json` с Bearer-токеном (файл в `.gitignore`), добавляет **`atlas-grid-dev`** (stdio) и напоминает сделать **Reload** MCP в Cursor.
...
### Два MCP-сервера в Cursor

| Сервер | Тип | Когда использовать |
|--------|-----|-------------------|
| **`atlas-grid`** | HTTP (`/api/v1/mcp/`) | Живые данные: проекты, POI, jobs, анализ, wiki resources |
| **`atlas-grid-dev`** | stdio (локальный процесс) | Разработка: pytest, поиск в коде, git — см. [§11](#11-dev-stdio-mcp-фаза-4) |

Оба сервера в одном `.cursor/mcp.json`. Скрипт `get-atlas-grid-token.ps1` настраивает HTTP MCP и по умолчанию добавляет `atlas-grid-dev` (отключить: `-NoDevMcp`).
...
### Типичные ошибки HTTP MCP

| Симптом | Причина | Решение |
|---------|---------|---------|
| 401 Unauthorized | Токен истёк (~60 мин) или неверный Bearer | `.\scripts\get-atlas-grid-token.ps1` → Reload MCP |
| 401 после «успешного» URL | URL без trailing slash → 307, Cursor теряет `Authorization` | URL должен быть `.../api/v1/mcp/` |
| `${env:TOKEN}` не работает | Ограничение Cursor на Windows | Токен вшивать скриптом, не через env в headers |
| Connection refused (local) | Backend не запущен | `python run_local.py` в `decision-matrix/backend` |
...
Конфиг: `ASSISTANT_WIKI_ENABLED`, `ASSISTANT_WIKI_ROOT`, `ASSISTANT_WIKI_MAX_ARTICLE_CHARS`, `ASSISTANT_WIKI_RAG_ENABLED`.

**Embeddings (отдельно от chat LLM):** `ASSISTANT_WIKI_EMBEDDING_BASE_URL`, `ASSISTANT_WIKI_EMBEDDING_API_KEY`, `ASSISTANT_WIKI_EMBEDDING_MODEL` — если пусто, используются `ASSISTANT_LLM_*`. Runtime override через admin UI (`/admin/assistant`, вкладка Wiki RAG).

**v1:** keyword search. **v2 (RAG):** гибрид keyword + vector (`knowledge/rag.py`) — embeddings через `/embeddings`, fallback на keyword/TF-IDF. `search_wiki` → поле `mode` в ответе; status → `wiki_rag_*`. Диагностика embeddings — `POST /admin/assistant/llm-probe`.
...
Из корня репозитория (HTTP MCP + dev stdio по умолчанию):

```powershell
.\scripts\get-atlas-grid-token.ps1
```

Только HTTP MCP (без `atlas-grid-dev`):

```powershell
.\scripts\get-atlas-grid-token.ps1 -NoDevMcp
```

Или вручную из [`.cursor/mcp.json.example`](../../.cursor/mcp.json.example) — **обязательно** `PYTHONPATH` (см. ниже):
...
**Важно (Windows / Cursor):** поле `cwd` в `mcp.json` для stdio MCP **может не применяться** — процесс стартует из корня workspace. Без `PYTHONPATH` в `env` Cursor падает с `ModuleNotFoundError: No module named 'app'`. Скрипт `get-atlas-grid-token.ps1` задаёт `PYTHONPATH` автоматически.

После правки конфига: **Settings → Tools & MCP → Reload**.

Проверка: в логе `%APPDATA%\Cursor\logs\*\mcp-server-*-atlas-grid-dev.log` не должно быть `No module named 'app'`.
