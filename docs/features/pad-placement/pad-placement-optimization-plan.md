...
- [x] [pad-placement-optimization.md](pad-placement-optimization.md)
...
1. **`normalize_bottomholes(snapshot) → list[LogicalWell]`**
   ...
   - ГС: группировка по `gs_heel_id`; Т3 подтягивается из snapshot.
...
- UI — [журнал задач](../jobs/task-log-panel.md).
