...
> **Спецификация:** [pad-placement-optimization.md](pad-placement-optimization.md).
...
|------|-----|-------|----------|
| `bottomhole_ids` | `UUID[]` | да | Id выбранных забоев (ННБ, Т1 ГС; Т3 подтягивается по `gs_heel_id`) |
...
| `bottomhole_ids` | UUID[] | 1 id для ННБ; 2 для ГС (Т1, Т3) |
| `td_longitude` | number | Долгота TD (для ГС — Т3, для ННБ — точка забоя) |
...
| `heel_longitude` | number | Только ГС — Т1 |
| `heel_latitude` | number | Только ГС — Т1 |
...
При `gs_entry_search_step_m: null` и `gs_entry_mode: any` backend вычисляет шаг по длине горизонтали (Т1→Т3 в plan ENU):
