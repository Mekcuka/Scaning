...
**Связанные функции:** [Траектории скважин (3D)](well-trajectory.md) — **M2 ✅, M3 ✅**: «Кустование», design до забоев, **anti-collision SF**, GeoJSON на карте; координаты устьев — `pad_wells_local_json`. [Оптимизация размещения кустов](pad-placement-optimization.md) (✅) — переиспользует **генератор раскладки** (`generate-from-wells`) для каждого кандидата куста; M2+ — **двухфазный** перебор центра по Σ MD. [Оценка](../planning/well-trajectory-app-assessment.md). [Roadmap](../planning/well-trajectory-roadmap.md).
