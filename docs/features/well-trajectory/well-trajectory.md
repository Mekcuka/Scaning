...
**Расчётный модуль:** [`well-trajectory-planner`](../../well-trajectory-planner/) на базе [welleng](https://github.com/jonnymaserati/welleng) и [PyWellGeo](https://github.com/TNO/pywellgeo). Техническое описание сервиса — [MICROSERVICE.md](../../well-trajectory-planner/docs/MICROSERVICE.md). Сравнение «что используем / что доступно» в библиотеках — [well-trajectory-libraries-coverage.md](well-trajectory-libraries-coverage.md).

**Связано с:** [земляные работы куста](../pad-earthwork/pad-earthwork.md) (раскладка устьев на плане), [3D-карта](../map/map-3d-features.md), [каталог параметров](../../product/input-parameters.md), **[оценка текущего приложения](well-trajectory-app-assessment.md)**, **[оптимизация размещения кустов](../pad-placement/pad-placement-optimization.md)** (✅ — проектный расчёт координат кустов по забоям; отличие от per-pad «Кустование» ниже).
...
> **Подробная оценка всех настроек, defaults и пробелов:** [well-trajectory-app-assessment.md](well-trajectory-app-assessment.md).

### Уже реализовано

| Область | Что есть |
|---------|----------|
| **Раскладка устьев на кусте** | `pad_well_count`, `pad_wells_local_json`, генератор схемы — план, без глубины ([pad-earthwork.md](../pad-earthwork/pad-earthwork.md)) |
| **Страница «Кустование»** | `/pad-clustering`: три subnav — **Куст**, **Сводка расчёта** (единая read-only таблица), **Профиль траектории** (MD–TVD, станции, Excel, маркеры Т1/Т3 для ГС); legacy `/pad-clustering/{projectId}` → **Куст** |
| **BFF well-trajectory** | `generate-from-layout`, `sync-bottomholes`, `design-from-bottomholes`, `GET last`, GeoJSON (куст + проект) |
| **Забои на карте** | `well_bottomhole_*`, рисование ННБ/ГС, привязка к кусту, sequential `well_index`; цепочка **основной забой → доп.ствол** (`role` / `parent_id`) |
| **2D / 3D слои** | GeoJSON на `/map`: plan line, маркеры забоев, траектории; 3D — `MapView3D` |
| **Настройки welleng на кусте** | 8 параметров на вкладке «Расчёт» — [таблица §4.5](well-trajectory-app-assessment.md#45-настройки-расчёта-welleng--pywellgeo-вкладка-расчёт) |
...
| **ГС: точка входа** | Режимы `any` / `Т1` / `Т3` на Т1 ГС; при `any` — min MD + учёт SF vs другие скважины куста |
| **ГС: dual TVD** | Отдельные TVD Т1 и Т3 (`heel_tvd_m` / `toe_tvd_m`); Z в геометрии ↔ TVD в параметрах |
...
- WITSML 1.4/2.0 (фаза 4b) — [ADR draft](well-trajectory-witsml-adr.md)
...
2. На **2D-карте** в панели рисования выбрать **«Забой»** → **ННБ** (один клик) или **ГС** (линия Т1→Т3; legacy — два клика Т1 + Т3).
...
4. В карточке объекта-забоя: куст, **геометрия** (координаты X/Y, отметка Z), TVD / inc / azi; для ГС — **точка входа** (`Любая` / `Т1` / `Т3`), отдельные TVD Т1 и Т3; кнопка «Пересчитать траекторию с куста». Вкладки **«Основное»**, **«Геометрия»** и **«3D»** — без «Логистики».
...
### 2c. Доп.ствол (цепочка «Куст → основной забой → доп.ствол»)

1. На карте создайте **основной** забой (`role=main`, по умолчанию) — ННБ или ГС — и привяжите к кусту.
2. Меню **«Забой»** → **Доп.ствол (ННБ)** или **Доп.ствол (ГС)** — система привяжет к ближайшему основному забою на том же кусте (`parent_id`). Альтернатива: в карточке объекта выставить роль «Доп.ствол» и выбрать родителя.
3. Доп.ствол **не** создаёт отдельный слот скважины и **не** участвует в «Рассчитать до забоев» как primary target основного ствола.
4. На **2D-карте:** фиолетовый маркер; пунктир от основного забоя к доп.стволу — **пока нет** рассчитанной ветки PyWellGeo.
5. На **«Кустование»** → вкладка **PyWellGeo** → боковой ствол **«До забоя»** → в списке целей группа **«Доп.стволы этой скважины»** (lateral с тем же `well_index`, что у основного забоя).
6. После расчёта ветки PyWellGeo пунктир parent→lateral на карте и в 3D кустования **скрывается** — остаётся линия траектории ветки (`pywellgeo_branch`).

### 2b. Горизонтальный участок ГС (Т1 → Т3)

Профиль `gs` в `design-from-bottomholes` строит траекторию в три логических сегмента (welleng connector):

1. **Build** — от устья до **точки входа** на линии Т1–Т3 (только у **Т1** или **Т3**).
2. **Hold** — горизонтальный участок вдоль оси ГС до противоположного конца (Т1→Т3 или Т3→Т1).

Внутренний вход между Т1 и Т3 и разворот на 180° у конца латерали **не используются** — при запросе промежуточного смещения точка входа приводится к ближайшему концу.

**Точка входа** (`well_bottomhole_gs_entry_mode` на Т1 / unified ГС):

| Режим | Поведение |
|-------|-----------|
| `Т1` | Вход у Т1; hold Т1 → Т3 |
| `Т3` | Вход у Т3; hold Т3 → Т1 |
| `any` (по умолчанию) | Сравниваются только вход у **Т1** и у **Т3**; выбирается **минимальная MD** (при равенстве — Т1). При расчёте SF на кусте — min MD среди вариантов с SF ≥ порога; если все нарушают SF — fallback на Т1, затем Т3 |

При разных TVD Т1 и Т3 hold следует наклону (интерполяция TVD и inc вдоль линии).
...
**Roadmap (не MVP):** pad-level lock или постановка design/clearance в очередь `ProjectJob` — см. [well-trajectory-app-assessment.md](well-trajectory-app-assessment.md).
...
Отдельная страница **«Кустование»** в навигации (`/pad-clustering`) — **три subnav-вкладки**:

| Subnav | Маршрут | Содержимое |
|--------|---------|------------|
| **Куст** | `/pad-clustering/workspace/{projectId}` | Sidebar «Куст / Расчёт / PyWellGeo» + 3D |
| **Сводка расчёта** | `/pad-clustering/summary/{projectId}` | Одна read-only таблица: **Раздел · Объект · Параметр · Значение** (куст, welleng, скважины, забои, PyWellGeo) |
| **Профиль траектории** | `/pad-clustering/profile/{projectId}` | График MD–TVD + таблица станций; селектор объекта в шапке страницы |

**Общий chrome** (все subnav): выбор куста, **Карта**, **Сохранить**, на вкладке «Профиль» — селектор **Объект** (`[Осн.] Скв-N` / `[Доп.] …`, query `?subject=` / `?well=`).

#### Сводка расчёта

- Плоский список строк без отдельных карточек по секциям.
- Пустые блоки (нет траекторий / забоев / PyWellGeo) — одна строка с подсказкой, что сделать на вкладке «Куст».
- Доп.стволы забоя в колонке «Объект» — с префиксом `↳`.

#### Профиль траектории

| Элемент | Описание |
|---------|----------|
| Селектор **Объект** | В шапке рядом с «Куст / Карта / Сохранить»; основной ствол (welleng survey) или доп.ствол (ветка PyWellGeo) |
| График **MD–TVD** | Recharts; для забоя **ГС** — маркеры **Т1** (зелёный) и **Т3** (красный) по TVD/plan и траектории |
| Таблица **Станции** | MD, TVD, наклон, азимут, DLS (°/30 м), N, E; **экспорт в Excel** (кнопка в заголовке таблицы) |
| Маркеры Т1/Т3 | Основной ствол — по ГС-забою скважины; доп.ствол — по ГС-забою доп.ствола (отдельная логика привязки по MD на горизонтальном участке) |

На вкладке **Куст** (sidebar):

| Вкладка sidebar | Содержимое |
|-----------------|------------|
| **Куст** | Раскладка, KB, генератор контура, pipeline траектории (заготовки → синхр. забоев → расчёт) |
| **Расчёт** | Параметры **welleng** и **pad-earthwork** — [§4.5 assessment](well-trajectory-app-assessment.md#45-настройки-расчёта-welleng--pywellgeo-вкладка-расчёт) |
| **PyWellGeo** | WellTree, YAML, AzimDip, thermal/DC1D — [wiki](../../wiki/articles/pad-clustering-pywellgeo.md) |
...
| Элемент | Описание |
|---------|----------|
| Слой «Забои скважин» | Маркеры `well_bottomhole_*` + GeoJSON `bottomhole_target`; **доп.ствол** — фиолетовый маркер |
| Инструмент «Забой» | ННБ / ГС; **Доп.ствол (ННБ/ГС)**; preview линии Т1–Т3 |
| Пунктир parent→lateral | `lateral-bottomhole-connector` — скрывается при наличии `pywellgeo_branch` до цели доп.ствола (см. §2c) |
| Пунктир устье–забой | `bottomhole_plan_line` до `design-from-bottomholes` |
| Ветки PyWellGeo | `pywellgeo_branch` на слое траекторий (фиолетовый оттенок) |

### 3D-карта

| Элемент | Описание |
|---------|----------|
| Слой «Траектории скважин» | Объёмные стволы (Three.js tubes) по `[lon, lat, z]`; вкл/выкл в «Слои» → «Траектории → 3D» |
| Цвет | **По SF:** красный если `min_sf` &lt; порога; иначе **палитра по `well_index`** (скважины различимы у куста) |
| Клик по линии | Панель: MD, inc, azi у ближайшей станции |
| Клик по маркеру забоя | Панель: TVD, inc/azi, «Спроектировать эту скважину» |

Формат GeoJSON — [модель данных §8](well-trajectory-data-model.md).
...
Ключи в `properties` куста — [input-parameters.md](../../product/input-parameters.md). Полная таблица welleng / PyWellGeo — [assessment §4.5](well-trajectory-app-assessment.md#45-настройки-расчёта-welleng--pywellgeo-вкладка-расчёт).
...
| `well_trajectory_inc_heel` | Inc на Т1 для ГС |
...
**Properties объектов-забоев** (см. [input-parameters.md](../../product/input-parameters.md)): `well_bottomhole_gs_entry_mode`, `well_bottomhole_heel_tvd_m`, `well_bottomhole_toe_tvd_m`, `well_bottomhole_role`, `well_bottomhole_parent_id`.

После `design-from-bottomholes` для ГС в `design` скважины: `gs_entry_mode`, `gs_entry_offset_m` (смещение от Т1, м), опц. `gs_entry_plan`.
...
| `POST /v1/design/horizontal` | ГС: build + hold, режимы `any` / `Т1` / `Т3` |
...
| Земляные работы | [pad-earthwork.md](../pad-earthwork/pad-earthwork.md) |
| 3D-карта | [map-3d-features.md](../map/map-3d-features.md) |
