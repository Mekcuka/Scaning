...
> **См. также:** [well-trajectory.md](well-trajectory.md), [well-trajectory-app-assessment.md](well-trajectory-app-assessment.md), [well-trajectory-roadmap.md](well-trajectory-roadmap.md), [pad-earthwork.md](../pad-earthwork/pad-earthwork.md), [input-parameters.md](../../product/input-parameters.md).
...
| `well_bottomhole_gs` | Линия Т1→Т3 (unified) | `linked_pad_id`, `heel_tvd_m`, `toe_tvd_m`, `gs_entry_mode` |
...
| `well_bottomhole_heel_tvd_m` | number | TVD Т1 ГС |
| `well_bottomhole_toe_tvd_m` | number | TVD Т3 ГС |
...
| `well_bottomhole_gs_heel_id` | UUID | Только на Т3 (legacy pair); ссылка на Т1 |
| `well_bottomhole_gs_entry_mode` | enum | `any` (default) / `Т1` / `Т3` — точка входа build→hold на линии Т1–Т3 |
| `well_bottomhole_role` | enum | `main` (default) / `lateral` — основной забой или доп.ствол |
| `well_bottomhole_parent_id` | UUID | Для `role=lateral`: ссылка на **основной** забой; куст и `well_index` наследуются |

**Цепочка «Куст → Забой → Доп.ствол»:** основной забой привязывается к кусту (`linked_pad_id`); доп.стволы (ННБ или ГС) ссылаются на основной через `parent_id` и **не** участвуют в `sync-bottomholes` как primary target — они используются как цели боковых веток PyWellGeo («До забоя»).

**Правила ролей:**

| Роль | `linked_pad_id` | `parent_id` | `well_index` | sync / slot_demand |
|------|-----------------|-------------|--------------|-------------------|
| `main` (default) | задаётся на куст | запрещён | auto или явный | участвует |
| `lateral` | наследуется от parent | обязателен → main | наследуется от parent | **не** участвует |

**Отображение на карте:** пунктир parent→lateral (`lateral-bottomhole-connector`) **не рисуется**, если для этой пары уже есть ветка PyWellGeo (`kind=pywellgeo_branch` в GeoJSON): конец ветки совпадает с целью доп.ствола в пределах **75 м** (тот же куст, тот же `well_index`). Frontend: `lateralBottomholeIdsWithBranchCoverage`, `buildLateralBottomholeConnectors({ excludeLateralIds })`.

**Обратная совместимость:** объекты без `well_bottomhole_role` трактуются как `main`; автомиграция «лишних» NNB в lateral не выполняется.
...
**Pad placement (оптимизация кустов):** тот же `design_well_from_target`, но `entry_clearance=false` и адаптивный шаг entry; SF — один раз на вариант при `sf_check`. См. [pad-placement-optimization.md](../pad-placement/pad-placement-optimization.md).
...
|------|-----------|
| `profile` | `"gs"` |
| `heel_plan`, `plan` | План Т1 и Т3 (east_m, north_m) |
...
## 9.5 PyWellGeo tree (вкладка «PyWellGeo»)

Per-pad JSON в `properties` (без миграций БД). Welleng `survey.stations` остаётся каноном для clearance; дерево WellTree — для ветвей, AHD, thermal.

| Ключ | Содержимое |
|------|------------|
| `pad_pywellgeo_settings_json` | `{ default_radius_m, tsurface_c, tgrad_c_per_m, yaml_format_default }` |
| `pad_pywellgeo_trees_json` | `[{ well_index, name?, tree: { nodes… branches[] }, source?, geometry?, branch_stats? }]` |
| `pad_pywellgeo_last_computed_at` | ISO timestamp |

В `pad_wells_trajectories_json[i]` опциональный блок **`pywellgeo`** (`geometry`, `branch_stats`) после `POST .../pywellgeo/apply-to-geometry`.

BFF: `GET/PUT .../pywellgeo/last|trees`, `POST .../sync-from-survey`, `POST .../compute`, `GET .../plot-data`. Planner: `/v1/pywellgeo/*`. ADR GPL: [adr-pywellgeo-gpl.md](../pad-earthwork/adr-pywellgeo-gpl.md).

---

## 10. Сводка API (черновик)

Подробнее — [well-trajectory.md](well-trajectory.md) и [MICROSERVICE.md](../../well-trajectory-planner/docs/MICROSERVICE.md).
