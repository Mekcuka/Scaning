...
При обычном импорте CSV/GeoJSON каждая линия проверяется: концы должны **совпадать** с координатами точечного объекта инфраструктуры (см. `line_endpoint_rules.py`, `coords_equal`). При create/update концы записываются как `lon`/`lat` найденного объекта; координаты вершин в БД не округляются до 3 знаков.
...
- [map-objects-and-spatial-calculations.md](../map/map-objects-and-spatial-calculations.md) — подтипы на карте
