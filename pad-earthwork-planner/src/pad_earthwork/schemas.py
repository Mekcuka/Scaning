...
class ProfileChainagePoint(BaseModel):
    chainage_m: float
    elevation_m: float


class ProfileSketch(BaseModel):
    kind: Literal["profile"] = "profile"
    width_m: float = Field(..., gt=0, le=500)
    chainage_points: list[ProfileChainagePoint] = Field(default_factory=list)
    design_elevation_m: float = Field(..., ge=-500, le=5000)


SketchInput = Annotated[
    PlanRectangleSketch | PlanPolygonSketch | ProfileSketch,
